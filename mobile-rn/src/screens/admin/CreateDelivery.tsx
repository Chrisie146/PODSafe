...
import { createUploadToken, generateUploadLink } from '../../repositories/externalUploadService';
...
export default function CreateDelivery({ route, navigation }: CreateDeliveryProps) {
  ...
  const [scheduledDateText, setScheduledDateText] = useState(new Date().toISOString().slice(0, 10));
  const [invoiceDateText, setInvoiceDateText] = useState('');
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(null);
  const [selectedCustomerId, setSelectedCustomerId] = useState<string | undefined>(undefined);
  const [selectedCustomerNumber, setSelectedCustomerNumber] = useState<string | undefined>(undefined);

  const [items, setItems] = useState<DeliveryItem[]>([]);
  const [itemDialog, setItemDialog] = useState<{ index: number | null } | null>(null);
  ...
  useEffect(() => {
    ...
    authRepository
      ...
      .then((users) => setDrivers(users.filter((u) => u.approvalStatus !== 'rejected')))
      .catch(() => setDrivers([]));
  ...
  useEffect(() => {
    ...
    const unsubscribe = vehicleRepository.subscribeToVehicles(
      ...
      (allVehicles) => {
        ...
        setSelectedVehicle((prev) => (prev && active.some((v) => v.registration === prev) ? prev : active[0]?.registration));
      },
      () => setIsLoadingVehicles(false),
    );
    return unsubscribe;
  ...
  useEffect(() => {
    ...
    setScheduledDateText(selectedDelivery.scheduledDate.toISOString().slice(0, 10));
    setInvoiceDateText(selectedDelivery.invoiceDate ? selectedDelivery.invoiceDate.toISOString().slice(0, 10) : '');
    setSelectedCustomerNumber(selectedDelivery.customerNumber);
    setItems(selectedDelivery.items);
    setSelectedDriverId(selectedDelivery.driverId === 'third-party' ? undefined : selectedDelivery.driverId);
  ...
  const handleSave = async () => {
    ...
    try {
      const delivery: Delivery = {
        ...
        invoiceDate,
        items,
        vehicleUsed: isThirdPartyTransport ? undefined : selectedVehicle ? normalizeRegistration(selectedVehicle) : undefined,
        isThirdPartyTransport,
        thirdPartyProviderName: isThirdPartyTransport ? thirdPartyProvider.trim() : undefined,
        thirdPartyDriverName: isThirdPartyTransport ? thirdPartyDriver.trim() || undefined : undefined,
        thirdPartyDriverPhone: isThirdPartyTransport ? thirdPartyPhone.trim() || undefined : undefined,
        thirdPartyVehicleInfo: isThirdPartyTransport ? thirdPartyVehicle.trim() || undefined : undefined,
        status: 'pending',
        scheduledDate,
        createdAt: isEditing && selectedDelivery ? selectedDelivery.createdAt : new Date(),
        notes: notes.trim() || undefined,
      };

      if (isEditing) {
        const ok = await updateDelivery(delivery);
        if (!ok) throw new Error(useDeliveryStore.getState().errorMessage ?? 'Failed to update delivery');
      ...
      if (!ok) throw new Error(useDeliveryStore.getState().errorMessage ?? 'Failed to create delivery');
      const newDeliveryId = useDeliveryStore.getState().selectedDelivery?.id;
      if (!newDeliveryId) throw new Error('Delivery created but no ID was returned');
  ...
  return (
    <View style={styles.container}>
      <StepIndicator currentStep={currentStep} onStepTap={(step) => step < currentStep && setCurrentStep(step)} />

      {currentStep === 0 ? (
        <ScrollView contentContainerStyle={styles.stepContent}>
          <Text style={textStyles.heading3}>Who are we delivering to?</Text>
          <View style={styles.gap} />
          <CustomerAutocomplete initialCustomer={selectedCustomer} onCustomerSelected={handleSelectCustomer} labelText="Search Customer" hintText="Search by name or customer number..." />
          <View style={styles.gap} />
          <LabeledInput label="Customer Name *" value={customerName} onChangeText={setCustomerName} />
          {selectedCustomer ? <Text style={styles.helperTextOk}>From: {selectedCustomer.customerNumber}</Text> : <Text style={styles.helperText}>Or type a name manually</Text>}
          <View style={styles.gap} />
          <LabeledInput label="Delivery Address *" value={customerAddress} onChangeText={setCustomerAddress} multiline />
          <View style={styles.gap} />
          <LabeledInput label="Phone Number (Optional)" value={customerPhone} onChangeText={setCustomerPhone} keyboardType="phone-pad" />
          <View style={styles.gapLarge} />
          <View style={styles.stepButtonRow}>
            <Pressable style={styles.primaryButton} onPress={handleNextStep}>
              <Text style={textStyles.buttonText}>Next: Items →</Text>
            </Pressable>
          </View>
        </ScrollView>
      ) : null}
        ...
        <View style={styles.stepFlexColumn}>
          {customerName ? (
            <View style={styles.bannerBar}>
              <Text style={styles.bannerText}>Delivering to: {customerName}</Text>
            </View>
          ) : null}
          <ScrollView contentContainerStyle={styles.stepContent}>
            <View style={styles.rowBetween}>
              <Text style={textStyles.heading3}>What are we delivering?</Text>
              <Pressable style={styles.smallPrimaryButton} onPress={() => setItemDialog({ index: null })}>
                <Text style={styles.smallPrimaryButtonText}>+ Add Item</Text>
              </Pressable>
            </View>
            <View style={styles.gap} />
            {items.length === 0 ? (
              <View style={[styles.card, shadows.card, styles.emptyItemsCard]}>
                ...
                <Text style={styles.emptyItemsHint}>Tap &quot;Add Item&quot; to add products to this delivery</Text>
              </View>
            ) : (
              items.map((item, index) => (
                <View key={index} style={[styles.card, shadows.card, styles.itemRow]}>
                  <View style={styles.itemQtyBadge}>
                    <Text style={styles.itemQtyText}>{item.quantity % 1 === 0 ? item.quantity.toString() : item.quantity.toFixed(2)}</Text>
                  </View>
                  <View style={styles.itemTextBox}>
                    <Text style={styles.itemDescription}>{item.description}</Text>
                    {item.unit || item.unitPrice != null ? (
                      <Text style={styles.itemSubtitle}>
                        {[item.unit, item.unitPrice != null ? `R${item.unitPrice.toFixed(2)} each` : undefined].filter(Boolean).join(' · ')}
                      </Text>
                    ) : null}
                  </View>
                  <Pressable onPress={() => setItemDialog({ index })}>
                    <Text style={styles.itemActionIcon}>✎</Text>
                  </Pressable>
                  <Pressable onPress={() => setItems((prev) => prev.filter((_, i) => i !== index))}>
                    <Text style={[styles.itemActionIcon, styles.itemActionDelete]}>🗑</Text>
                  </Pressable>
                </View>
              ))
            )}
          ...
          <View style={styles.stepButtonRow}>
            <Pressable style={styles.secondaryButton} onPress={handlePrevStep}>
              <Text style={styles.secondaryButtonText}>← Back</Text>
            </Pressable>
            <View style={styles.spacer} />
            {items.length > 0 ? <Text style={styles.itemCountText}>{items.length} item{items.length === 1 ? '' : 's'}</Text> : null}
            <Pressable style={styles.primaryButton} onPress={handleNextStep}>
              <Text style={textStyles.buttonText}>Next: Schedule →</Text>
            </Pressable>
          </View>
        </View>
      ) : null}
      ...
      {currentStep === 2 ? (
        <View style={styles.stepFlexColumn}>
          ...
          <ScrollView contentContainerStyle={styles.stepContent}>
            ...
            <View style={styles.gap} />
            <LabeledInput label="Scheduled Date * (YYYY-MM-DD)" value={scheduledDateText} onChangeText={setScheduledDateText} />
            <Text style={styles.helperText}>{formatLongDate(scheduledDate)}</Text>
            <View style={styles.gapLarge} />

            <Text style={styles.fieldLabel}>Transport Method</Text>
            <View style={styles.gap} />
            <View style={[styles.card, shadows.card]}>
              <RadioRow label="Own Fleet" subtitle="Assign to your driver and vehicle" selected={!isThirdPartyTransport} onPress={() => setIsThirdPartyTransport(false)} />
              <Divider />
              <RadioRow label="Third-Party Transport" subtitle="External transport company" selected={isThirdPartyTransport} onPress={() => setIsThirdPartyTransport(true)} />
            </View>
            <View style={styles.gapLarge} />
            ...
            {!isThirdPartyTransport ? (
                ...
                {drivers === null ? (
                  ...
                  <View style={styles.chipRow}>
                    {drivers.map((driver) => (
                      <Pressable
                        key={driver.id}
                        style={[styles.chip, selectedDriverId === driver.id && styles.chipSelected]}
                        onPress={() => handleSelectDriver(driver.id)}
                      >
                        <Text style={[styles.chipText, selectedDriverId === driver.id && styles.chipTextSelected]}>{driver.fullName}</Text>
                      </Pressable>
                    ))}
                  </View>
                )}
                {selectedDriverId ? (
                  <>
                    ...
                    {isLoadingVehicles ? (
                      ...
                      <Text style={styles.helperText}>No active vehicles available</Text>
                    ) : (
                      <>
                        ...
                        <View style={styles.chipRow}>
                          {vehicles.map((vehicle) => {
                            ...
                            return (
                              <Pressable
                                key={vehicle.id}
                                style={[styles.chip, selectedVehicle === vehicle.registration && styles.chipSelected]}
                                onPress={() => setSelectedVehicle(vehicle.registration)}
                              >
                                <Text style={[styles.chipText, selectedVehicle === vehicle.registration && styles.chipTextSelected]}>
              ...
              <View style={[styles.card, shadows.card, styles.thirdPartyCard]}>
                <Text style={styles.thirdPartyTitle}>🚚 Third-Party Transport Details</Text>
                <View style={styles.gap} />
                <LabeledInput label="Provider Name *" value={thirdPartyProvider} onChangeText={setThirdPartyProvider} placeholder="e.g., HFR Transport" />
                <View style={styles.gap} />
                <LabeledInput label="Driver Name (Optional)" value={thirdPartyDriver} onChangeText={setThirdPartyDriver} />
                <View style={styles.gap} />
                <LabeledInput label="Driver Phone (Optional)" value={thirdPartyPhone} onChangeText={setThirdPartyPhone} keyboardType="phone-pad" />
                <View style={styles.gap} />
                <LabeledInput label="Vehicle Info (Optional)" value={thirdPartyVehicle} onChangeText={setThirdPartyVehicle} placeholder="Registration or description" />
                <View style={styles.gap} />
                <View style={styles.infoNote}>
                  <Text style={styles.infoNoteText}>ℹ️ An upload link will be generated for this provider to submit POD documents.</Text>
                </View>
              </View>
            )}
            <View style={styles.gapLarge} />

            <LabeledInput label="Order Number (Optional)" value={orderNumber} onChangeText={setOrderNumber} />
            <View style={styles.gap} />
            <LabeledInput label="Invoice Date (YYYY-MM-DD, Optional)" value={invoiceDateText} onChangeText={setInvoiceDateText} />
            <View style={styles.gap} />
            ...
            <Pressable style={styles.saveButton} disabled={isSaving} onPress={handleSave}>
              {isSaving ? <ActivityIndicator color={colors.white} /> : <Text style={textStyles.buttonText}>{isEditing ? 'Update Delivery' : 'Create Delivery'}</Text>}
            </Pressable>
          </ScrollView>
          <View style={styles.stepButtonRow}>
            <Pressable style={styles.secondaryButton} onPress={handlePrevStep}>
      ...
      {itemDialog ? (
        <ItemDialog
          existingItem={itemDialog.index != null ? items[itemDialog.index] : null}
      ...
      {instructionsModal ? (
        <Modal visible transparent animationType="fade" onRequestClose={() => setInstructionsModal(null)}>
          <View style={styles.modalBackdrop}>
            <View style={[styles.modalCard, shadows.card]}>
              <Text style={textStyles.heading3}>ℹ️ Delivery Instructions</Text>
              <Text style={styles.helperText}>Special instructions for this customer:</Text>
              <View style={styles.infoNote}>
                <Text style={styles.instructionsText}>{instructionsModal}</Text>
              </View>
              <Pressable style={styles.modalCloseButton} onPress={() => setInstructionsModal(null)}>
        ...
        <Modal visible transparent animationType="fade">
          <View style={styles.modalBackdrop}>
            <View style={[styles.modalCard, shadows.card]}>
              ...
              <Text style={styles.helperText}>Delivery created successfully! Share this link with the transport provider to upload POD documents:</Text>
              <View style={styles.infoNote}>
...
function StepIndicator({ currentStep, onStepTap }: { currentStep: number; onStepTap: (step: number) => void }) {
  return (
    <View style={styles.stepIndicatorRow}>
      <StepChip step={0} label="Customer" currentStep={currentStep} onPress={onStepTap} />
      <View style={[styles.stepDivider, currentStep > 0 && styles.stepDividerActive]} />
      <StepChip step={1} label="Items" currentStep={currentStep} onPress={onStepTap} />
      <View style={[styles.stepDivider, currentStep > 1 && styles.stepDividerActive]} />
      <StepChip step={2} label="Schedule" currentStep={currentStep} onPress={onStepTap} />
    </View>
  );
}

function StepChip({ step, label, currentStep, onPress }: { step: number; label: string; currentStep: number; onPress: (step: number) => void }) {
  const isActive = currentStep === step;
  const isCompleted = currentStep > step;
  const color = isCompleted ? colors.success : isActive ? colors.primary : colors.divider;
  return (
    <Pressable style={styles.stepChipContainer} onPress={() => onPress(step)}>
      <View style={[styles.stepCircle, { backgroundColor: color }]}>
        <Text style={styles.stepCircleText}>{isCompleted ? '✓' : step + 1}</Text>
      </View>
      <Text style={[styles.stepLabel, { color }, isActive && styles.stepLabelActive]}>{label}</Text>
    </Pressable>
  );
}
...
function LabeledInput({
  ...
  onChangeText,
  ...rest
}: { label: string; value: string; onChangeText: (value: string) => void } & React.ComponentProps<typeof TextInput>) {
  return (
    <View>
      <Text style={styles.inputLabel}>{label}</Text>
      <TextInput style={[styles.input, rest.multiline && styles.inputMultiline]} value={value} onChangeText={onChangeText} {...rest} />
    </View>
  );
}
...
function ItemDialog({
  ...
  const [quantity, setQuantity] = useState(existingItem ? String(existingItem.quantity) : '1');
  const [unit, setUnit] = useState(existingItem?.unit ?? '');
  const [unitPrice, setUnitPrice] = useState(existingItem?.unitPrice != null ? existingItem.unitPrice.toFixed(2) : '');
  ...
  const handleSave = () => {
    ...
    const parsedUnitPrice = unitPrice.trim().length === 0 ? undefined : Number(unitPrice.trim());
    onSave({
      description: trimmedDescription,
      quantity: parsedQuantity,
      unit: unit.trim() || undefined,
      unitPrice: parsedUnitPrice,
      totalPrice: parsedUnitPrice != null ? parsedQuantity * parsedUnitPrice : undefined,
    });
  };
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onCancel}>
      <View style={styles.modalBackdrop}>
        <View style={[styles.modalCard, shadows.card]}>
          <Text style={textStyles.heading3}>{existingItem ? 'Edit Item' : 'Add Item'}</Text>
          <View style={styles.gap} />
          <LabeledInput label="Description *" value={description} onChangeText={setDescription} placeholder="Product or item description" autoFocus />
          <View style={styles.gap} />
          <View style={styles.rowGap}>
            <View style={styles.flex1}>
              <LabeledInput label="Quantity *" value={quantity} onChangeText={setQuantity} keyboardType="numeric" />
            </View>
            <View style={styles.flex1}>
              <LabeledInput label="Unit" value={unit} onChangeText={setUnit} placeholder="e.g., box, kg" />
            </View>
          </View>
          <View style={styles.gap} />
          <LabeledInput label="Unit Price (Optional)" value={unitPrice} onChangeText={setUnitPrice} keyboardType="decimal-pad" placeholder="0.00" />
          <View style={styles.modalActionsRow}>
            <Pressable style={styles.secondaryButton} onPress={onCancel}>
              <Text style={styles.secondaryButtonText}>Cancel</Text>
            </Pressable>
            <Pressable style={styles.modalCloseButton} onPress={handleSave}>
              <Text style={textStyles.buttonText}>{existingItem ? 'Save' : 'Add'}</Text>
            </Pressable>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  stepIndicatorRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, paddingVertical: spacing.medium, paddingHorizontal: spacing.large },
  ...
  stepDivider: { flex: 1, height: 2, backgroundColor: colors.divider, marginHorizontal: spacing.small },
  ...
  bannerBar: { backgroundColor: `${colors.primary}1A`, paddingHorizontal: spacing.medium, paddingVertical: spacing.small },
  ...
  input: { borderWidth: 1, borderColor: colors.divider, borderRadius: radii.borderRadius, paddingHorizontal: spacing.medium, paddingVertical: spacing.small + 4, backgroundColor: colors.card },
  inputMultiline: { minHeight: 70, textAlignVertical: 'top' },
  stepButtonRow: { flexDirection: 'row', alignItems: 'center', padding: spacing.medium, backgroundColor: colors.card, borderTopWidth: 1, borderTopColor: colors.divider },
  spacer: { flex: 1 },
  primaryButton: { backgroundColor: colors.primary, borderRadius: radii.buttonRadius, paddingHorizontal: spacing.large, paddingVertical: spacing.small + 4, alignItems: 'center' },
  ...
  smallPrimaryButton: { backgroundColor: colors.primary, borderRadius: radii.borderRadius, paddingHorizontal: spacing.medium, paddingVertical: spacing.small },
  smallPrimaryButtonText: { color: colors.white, fontWeight: '600', fontSize: 13 },
  ...
  emptyItemsText: { fontWeight: '600', color: colors.textSecondary, marginTop: spacing.small },
  emptyItemsHint: { fontSize: 12, color: colors.textSecondary, marginTop: 4, textAlign: 'center' },
  itemRow: { flexDirection: 'row', alignItems: 'center', marginBottom: spacing.small },
  itemQtyBadge: { width: 32, height: 32, borderRadius: 16, backgroundColor: `${colors.primary}1A`, alignItems: 'center', justifyContent: 'center', marginRight: spacing.medium },
  ...
  itemCountText: { fontSize: 13, color: colors.textSecondary, marginRight: spacing.medium },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.small, marginTop: spacing.small },
  chip: { borderWidth: 1, borderColor: colors.divider, borderRadius: radii.borderRadius, paddingHorizontal: spacing.medium, paddingVertical: spacing.small },
  chipSelected: { backgroundColor: colors.primary, borderColor: colors.primary },
  chipText: { fontSize: 13, color: colors.textSecondary },
  chipTextSelected: { color: colors.white, fontWeight: '600' },
  radioRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.small + 4 },
  radioGlyph: { fontSize: 20, color: colors.primary, marginRight: spacing.medium },
  radioTextBox: { flex: 1 },
  ...
  thirdPartyTitle: { fontWeight: 'bold', color: colors.warning, fontSize: 15 },
  infoNote: { backgroundColor: `${colors.info}14`, borderRadius: radii.borderRadius, padding: spacing.medium },
  infoNoteText: { color: colors.info, fontSize: 12 },
  saveButton: { backgroundColor: colors.success, borderRadius: radii.buttonRadius, paddingVertical: spacing.medium, alignItems: 'center' },
