...
/**
 * Ported from lib/screens/admin/claim_settings_desktop.dart (verified against source on
 * 2026-06-23). Confirmed by direct read that this screen's load/save logic and full field
 * set are identical to claim_settings_screen.dart's mobile variant already ported in
 * ClaimSettings.tsx (same `CompanyClaimSettings` model, same `useClaimStore`) — only the
 * chrome differs (250px side-nav instead of a top tab strip, 32px content padding). The
 * field state/handlers below are intentionally duplicated from ClaimSettings.tsx rather than
 * shared, consistent with this migration's per-screen-pair duplication convention (e.g.
 * VehicleManagement.tsx/VehicleManagementDesktop.tsx).
 *
 * Dropped (deliberate scope-trim, same precedent as Risk Register #14): Ctrl+S save /
 * Ctrl+1-6 section switch / Esc exit keyboard shortcuts, the AppBar's keyboard-hint chip, and
 * the "Unsaved changes" badge + Discard-with-confirmation flow (which needs per-field dirty
 * tracking across ~25 fields plus a `beforeRemove` navigation-intercept pattern that doesn't
 * exist anywhere else in this codebase yet). The Save button always works and always saves
 * the current form state — same behavior as the already-shipped mobile screen, just without
 * the extra "are you sure you want to leave" guard rail.
 */
const SECTIONS = [
  { key: 'general', label: 'General', icon: '⚙️' },
  { key: 'types', label: 'Claim Types', icon: '🗂️' },
...
export default function ClaimSettingsDesktop() {
      ...
      <View style={styles.body}>
        <View style={styles.sidebar}>
          {SECTIONS.map((section) => {
            ...
            return (
              <Pressable key={section.key} style={[styles.navItem, isActive && styles.navItemActive]} onPress={() => setActiveSection(section.key)}>
                <Text style={styles.navItemIcon}>{section.icon}</Text>
                <Text style={[styles.navItemLabel, isActive && styles.navItemLabelActive]}>{section.label}</Text>
              </Pressable>
            );
          })}
        </View>
            ...
            <View style={styles.columnsRow}>
              ...
              <View style={styles.column}>
                ...
                <View style={[styles.card, shadows.card]}>
                  ...
                  {enablePatternDetection ? (
                    <View style={styles.fieldGroupNoBottom}>
                      <View style={styles.divider} />
                      <FieldInput
...
const styles = StyleSheet.create({
  ...
  sidebar: { width: 250, backgroundColor: colors.card, borderRightWidth: 1, borderRightColor: colors.divider, paddingTop: spacing.medium },
  navItem: { flexDirection: 'row', alignItems: 'center', marginHorizontal: spacing.small, marginBottom: 4, paddingHorizontal: spacing.medium, paddingVertical: spacing.small + 4, borderRadius: radii.borderRadius },
  navItemActive: { backgroundColor: `${colors.primary}1A` },
  navItemIcon: { fontSize: 16, marginRight: spacing.small + 4 },
  navItemLabel: { fontSize: 14, fontWeight: '500', color: colors.textPrimary },
  navItemLabelActive: { color: colors.primary, fontWeight: '700' },
  content: { flex: 1 },
  contentInner: { padding: spacing.large, paddingBottom: spacing.xLarge },
  columnsRow: { flexDirection: 'row', gap: spacing.large },
  column: { flex: 1 },
  ...
  fieldGroupNoBottom: { marginBottom: 0 },
