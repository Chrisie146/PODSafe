...
import {
  getAbaserveCsvTemplate,
  ImportError,
  parseDeliveryCsv,
} from '../../repositories/bulkImportService';
...
import {
  AdminImportLanding,
  AdminImportLoading,
  AdminImportReviewSummary,
} from '../../components/admin/AdminImport';
import { colors, spacing, radii, shadows } from '../../theme/tokens';
import { textStyles } from '../../theme/textStyles';

/**
 * Ported from lib/screens/admin/abaserve_import_screen.dart (verified against source on
 * 2026-06-23) — near-duplicate of bulk_upload_screen.dart in the Dart source too (same
 * `BulkImportService`/`EditableDelivery` underneath, different template source and copy
 * for ABServe ERP-exported CSVs), so this mirrors BulkUpload.tsx's structure exactly
 * rather than introducing a shared component the Dart app itself doesn't have.
 *
 * Deviations from the Flutter source (same as BulkUpload.tsx):
 * - File picking/CSV reading/template download follow CustomerImport.tsx's conventions
 *   (@react-native-documents/picker + fetch().text(), exportToCSV's share sheet).
 * - The template is hardcoded in bulkImportService.ts's getAbaserveCsvTemplate() instead
 *   of bundled as a native text asset (Dart loads `assets/abaserve_import_template.csv`
 *   via `rootBundle` — no text-asset bundling exists elsewhere in this RN project yet, and
 *   this is a small, static, one-off template not worth the new asset-pipeline config).
 * - The scheduled-date cell is a plain `YYYY-MM-DD` text field instead of a native date
 *   picker, same as BulkUpload.tsx.
 * - Only the same subset of columns the Dart table exposes is editable (customer
...
export default function AbaserveImport() {
  ...
  const [driverEmailToId, setDriverEmailToId] = useState<
    Record<string, string>
  >({});
  const [driversLoaded, setDriversLoaded] = useState(false);
  const [existingInvoices, setExistingInvoices] = useState<string[]>([]);
  const [itemsEditorDelivery, setItemsEditorDelivery] =
    useState<EditableDelivery | null>(null);
  const [errorsModalDelivery, setErrorsModalDelivery] =
    useState<EditableDelivery | null>(null);
  ...
  useEffect(() => {
    ...
    deliveryRepository
      .getExistingInvoiceNumbers(currentUser.companyId)
  ...
  const pickFile = async () => {
    try {
      ...
      setIsLoading(true);
      setFileName(result.name ?? 'abserve_export.csv');
      ...
      const editableList = parseResult.validDeliveries.map(parsed =>
        EditableDelivery.fromParsed(parsed),
      );
      ...
      const errorCount =
        editableList.filter(d => d.hasErrors).length +
        parseResult.errors.length;
      const warningCount = editableList.reduce(
        (total, d) => total + d.warnings.length,
        0,
      );
      Alert.alert(
        'File Parsed',
  ...
  const downloadTemplate = async () => {
    try {
      const { headers, rows } = getAbaserveCsvTemplate();
      await exportToCSV('abaserve_import_template', headers, rows);
      Alert.alert('Success', '✓ Template downloaded successfully');
    } catch (e) {
      Alert.alert(
        'Error',
        `Error downloading template: ${(e as Error).message}`,
      );
    }
  };
  ...
  const updateRow = (
    ...
    mutate: (d: EditableDelivery) => void,
  ...
  const handleImport = async () => {
    ...
    const selectedValid = editableDeliveries.filter(
      d => d.isSelected && d.isValid && !d.isRemoved,
    );
          ...
          onPress: async () => {
            ...
            try {
              const deliveries = selectedValid.map(d => {
                const driverId =
                  d.driverEmail && driversLoaded
                    ? driverEmailToId[d.driverEmail.toLowerCase()] ??
                      'unassigned'
                    : 'unassigned';
                return d.toDelivery({
              ...
              Alert.alert(
                'Error',
                `Error importing deliveries: ${(e as Error).message}`,
              );
            }
          },
        },
  ...
  if (isLoading) {
    return (
      <View style={styles.centered}>
        <AdminImportLoading title="Preparing ABServe import" />
      </View>
    );
  }
  ...
  if (!editableDeliveries) {
    return (
      <AdminImportLanding
        title="Import from ABServe"
        description="Upload an ABServe ERP CSV, resolve validation issues, then create the selected delivery records."
        requirements={[
          'Required columns: customerName, customerAddress, invoiceNumber, and scheduledDate.',
          'Use YYYY-MM-DD dates, for example 2025-10-17.',
          'Optional columns include customer phone, driver email, and notes.',
        ]}
        note="Use the supplied ABServe template when preparing an export."
  ...
  const errorCount =
    activeDeliveries.filter(d => d.hasErrors).length + fileErrors.length;
  const warningCount = activeDeliveries.reduce(
    (total, d) => total + d.warnings.length,
    0,
  );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(activeDeliveries.length / ROWS_PER_PAGE),
  );
  ...
  return (
    <View style={styles.container}>
      <AdminImportReviewSummary
        ...
        allValidSelected={selectedCount === validCount && validCount > 0}
        onChooseNewFile={pickFile}
        onToggleAll={() =>
          toggleAllSelections(!(selectedCount === validCount && validCount > 0))
        }
      ...
      <ScrollView horizontal>
        <View style={styles.table}>
          <View style={styles.tableHeaderRow}>
            ...
            <Text style={[styles.headerCell, styles.colInvoice]}>
              Invoice #
            </Text>
            <Text style={[styles.headerCell, styles.colDate]}>Date</Text>
            <Text style={[styles.headerCell, styles.colDriver]}>
              Driver Email
            </Text>
      ...
      {totalPages > 1 ? (
        <View style={styles.paginationRow}>
          ...
          <Text style={styles.pageText}>
            Page {currentPage + 1} of {totalPages} (showing {startIndex + 1}-
            {endIndex} of {activeDeliveries.length})
          </Text>
          <Pressable
          ...
          >
            <Text
              style={[
                styles.pageArrow,
                currentPage >= totalPages - 1 && styles.pageArrowDisabled,
              ]}
            >
              ›
            </Text>
      ...
      {errorsModalDelivery ? (
        <Modal
          ...
          transparent
          animationType="fade"
          onRequestClose={() => setErrorsModalDelivery(null)}
        >
          <View style={styles.modalBackdrop}>
            <View style={[styles.modalCard, shadows.card]}>
              <Text style={textStyles.heading3}>
                Row #{errorsModalDelivery.rowNumber} - Issues
              </Text>
              ...
              <Pressable
                ...
                onPress={() => setErrorsModalDelivery(null)}
              >
      ...
      >
        <View style={styles.modalBackdrop}>
          <ScrollView style={[styles.modalCard, shadows.card]}>
            <Text style={textStyles.heading3}>ABServe Import Help</Text>
            <Text style={styles.helpSectionLabel}>
              How to use ABServe import:
            </Text>
            <Text style={styles.helpLine}>
              1. Export CSV from your ABServe system
            </Text>
            <Text style={styles.helpLine}>2. Upload the CSV file</Text>
            <Text style={styles.helpLine}>
              3. Review and edit data in the table
            </Text>
            <Text style={styles.helpLine}>4. Select rows to import</Text>
            <Text style={styles.helpLine}>5. Tap "Import Selected"</Text>
            ...
            <Text style={styles.helpLine}>
              • Tap items count to edit line items
            </Text>
            <Text style={styles.helpLine}>
              • Red rows have errors (cannot import)
            </Text>
            <Text style={styles.helpLine}>
              • Orange rows have warnings (can still import)
            </Text>
            <Text style={styles.helpLine}>• Blue rows have been edited</Text>
            <Text style={styles.helpLine}>
              • Use checkbox to select/deselect rows
            </Text>
            <Text style={styles.helpLine}>
              • Tap trash icon to remove from import
            </Text>
            <Pressable
              style={styles.modalCloseButton}
  ...
  return (
    <View style={[styles.tableRow, rowStyle]}>
      ...
      <View style={styles.colRow}>
        {delivery.hasErrors ? (
          <Text style={styles.rowIconError}>!</Text>
        ) : delivery.hasWarnings ? (
          <Text style={styles.rowIconWarning}>!</Text>
        ) : null}
      ...
      <TextInput
        style={[
          styles.cellInput,
          styles.colName,
          delivery.errors.some(e => e.field === 'customerName') &&
            styles.cellInputError,
        ]}
        value={delivery.customerName}
        onChangeText={value =>
          onChange(delivery, d => (d.customerName = value), true)
        }
      />
      <TextInput
        style={[
          styles.cellInput,
          styles.colAddress,
          delivery.errors.some(e => e.field === 'customerAddress') &&
            styles.cellInputError,
        ]}
        value={delivery.customerAddress}
        onChangeText={value =>
          onChange(delivery, d => (d.customerAddress = value), true)
        }
      />
      <TextInput
        ...
        onChangeText={value =>
          onChange(delivery, d => (d.customerPhone = value || undefined), true)
        }
      />
      <TextInput
        style={[
          styles.cellInput,
          styles.colInvoice,
          delivery.errors.some(e => e.field === 'invoiceNumber') &&
            styles.cellInputError,
        ]}
        value={delivery.invoiceNumber}
        onChangeText={value =>
          onChange(delivery, d => (d.invoiceNumber = value), true)
        }
      />
      <TextInput
        style={[
          styles.cellInput,
          styles.colDate,
          delivery.errors.some(e => e.field === 'scheduledDate') &&
            styles.cellInputError,
        ]}
        value={dateText}
        onChangeText={value => {
      ...
      <TextInput
        ...
        onChangeText={value =>
          onChange(delivery, d => (d.driverEmail = value || undefined), true)
        }
      />
      ...
      <TextInput
        ...
        onChangeText={value =>
          onChange(delivery, d => (d.notes = value || undefined), false)
        }
      />
    ...
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.modalBackdrop}>
        <View style={[styles.modalCard, shadows.card, styles.itemsModalCard]}>
          <Text style={textStyles.heading3}>
            Edit Items - Row #{delivery.rowNumber}
          </Text>
