...
class _DeliveryManagementDesktopState extends State<DeliveryManagementDesktop> {
  ...
  Map<String, dynamic>? _previewVehicleDetails; // Vehicle details for preview
                    ...
                    if (delivery.vehicleUsed != null && delivery.vehicleUsed!.isNotEmpty)
                      _buildPreviewSection(
                        ...
                        _loadingVehicleDetails
                            ...
                            : _previewVehicleDetails != null
                                ? [
                                    _buildPreviewRow('Registration', _previewVehicleDetails!['registration'] ?? delivery.vehicleUsed ?? '-'),
                                    if (_previewVehicleDetails!['make'] != null)
                                      _buildPreviewRow('Make', _previewVehicleDetails!['make'] ?? '-'),
                                    if (_previewVehicleDetails!['model'] != null)
                                      _buildPreviewRow('Model', _previewVehicleDetails!['model'] ?? '-'),
                                    if (_previewVehicleDetails!['color'] != null)
                                      _buildPreviewRow('Color', _previewVehicleDetails!['color'] ?? '-'),
                                    if (_previewVehicleDetails!['plateNumber'] != null)
                                      _buildPreviewRow('Plate', _previewVehicleDetails!['plateNumber'] ?? '-'),
                                    if (_previewVehicleDetails!['year'] != null)
                                      _buildPreviewRow('Year', _previewVehicleDetails!['year'].toString()),
                                  ]
                                : [
                                    ...
                                    const Padding(
                                      ...
                                      child: Text(
                                        'Vehicle details not found in system',
  ...
  void _selectDeliveryForPreview(Delivery delivery) {
    setState(() {
      ...
      _previewVehicleDetails = null;
      _loadingVehicleDetails = false;
    });
    
    // Load vehicle details if vehicle is assigned
    if (delivery.vehicleUsed != null && delivery.vehicleUsed!.isNotEmpty) {
      _loadVehicleDetails(delivery.vehicleUsed!);
    }
  }
  
  Future<void> _loadVehicleDetails(String vehicleRegistration) async {
    try {
      setState(() => _loadingVehicleDetails = true);
      ...
      debugPrint('🚗 Loading vehicle details for: $vehicleRegistration');
      
      // Normalize registration (uppercase, remove extra spaces)
      final normalized = vehicleRegistration.trim().toUpperCase();
      ...
      var snapshot = await FirebaseFirestore.instance
          ...
          .doc(companyId)
          .collection('vehicles')
          .where('registration', isEqualTo: normalized)
          .limit(1)
          .get();
      
      // If no exact match, try case-insensitive by getting all and filtering
      if (snapshot.docs.isEmpty) {
        debugPrint('⚠️ No exact match for: $normalized, trying all vehicles...');
        ...
        for (var doc in allVehicles.docs) {
          final reg = doc.data()['registration']?.toString().trim().toUpperCase();
          if (reg == normalized) {
            // Found matching vehicle
            setState(() {
              _previewVehicleDetails = doc.data();
              _loadingVehicleDetails = false;
            });
            debugPrint('✅ Vehicle details loaded: ${doc.id}');
            return;
          }
        }
      }
      ...
      if (snapshot.docs.isNotEmpty) {
        setState(() {
          _previewVehicleDetails = snapshot.docs.first.data();
          _loadingVehicleDetails = false;
        });
        debugPrint('✅ Vehicle details loaded: ${snapshot.docs.first.id}');
      } else {
        debugPrint('⚠️ No vehicle found for registration: $vehicleRegistration');
        setState(() => _loadingVehicleDetails = false);
      }
    } catch (e) {
